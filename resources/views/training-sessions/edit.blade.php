    ...
    <form action="{{ route('training-sessions.update', $trainingSession) }}" method="POST" class="training-form">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="date" name="session_date" id="session_date" value="{{ old('session_date', $trainingSession->session_date->format('Y-m-d')) }}" required>
                @error('session_date')
                    <span class="error">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="time" name="session_time" id="session_time" value="{{ old('session_time', \Carbon\Carbon::parse($trainingSession->session_time)->format('H:i')) }}" required>
                @error('session_time')
                    <span class="error">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" name="duration_minutes" id="duration_minutes" value="{{ old('duration_minutes', $trainingSession->duration_minutes) }}" min="1" required>
        ...
        <div class="form-group">
            ...
            <textarea name="description" id="description" rows="3" placeholder="Brief description of your workout">{{ old('description', $trainingSession->description) }}</textarea>
            @error('description')
                <span class="error">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <textarea name="notes" id="notes" rows="4" placeholder="How did you feel? Any achievements or challenges?">{{ old('notes', $trainingSession->notes) }}</textarea>
            @error('notes')
                <span class="error">{{ $message }}</span>
            @enderror
        </div>
